...
{ 
    ...
    {   public int state;
        ...
        private void teachername_Validating(object sender, CancelEventArgs e)
        {
            if (!Regex.IsMatch(teachername.Text, @"(([А-Я][а-я]+(\s?))+$)|([А-Я][а-я]+(-?)([А-Я]?)([а-я]+)$)"))
            {
                teachername.Tag = false;
                    ...
                    if (state == 1)
                    {
                        MessageBox.Show("Происходи создание и редактирование объекта");
                    }
            }
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            foreach (all i in alls.alls)
            {
                str.Append(i.subject.subjectname +"\r\n");
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            Teacher adapte = new Teacher("Н.В.Пацей", 105, "Информационных систем и технологий", "10521445");
            Target target = new Adapter(adapte);
            MessageBox.Show(target.StartTeach());
        ...
        private void department_Validated(object sender, EventArgs e)
        {
            if (department.Text != "") { 
            state++;
            if(state==1)
        ...
        private void auditorium_Validated(object sender, EventArgs e)
        {
            if (auditorium.Value !=0)
            {
                state++;
                if (state == 1)
                {
        ...
        private void telephone_Validated(object sender, EventArgs e)
        {
            if (telephone.Text!= "")
            {
                state++;
